...
class Client(Base):
    ...
    short_name: Mapped[str | None] = mapped_column(
        String(100)
    )  # Сокращённое наименование клиента
    type: Mapped[ClientType] = mapped_column(
        SQLEnum(ClientType, native_enum=False), nullable=False
    )  # Тип клиента (физ/юр лицо/суд)
...
class Contact(Base):
    ...
    name: Mapped[str] = mapped_column(String(255), nullable=False)  # Имя контакта
    position: Mapped[str | None] = mapped_column(String(255))  # Должность контакта
    ...
    contact_type: Mapped[ContactType] = mapped_column(
        SQLEnum(ContactType, native_enum=False), nullable=False
    )  # Тип контакта
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now()
    )  # Дата создания
    ...
    client: Mapped[Client] = relationship(
        "Client", back_populates="contacts"
    )  # Ссылка на клиента
